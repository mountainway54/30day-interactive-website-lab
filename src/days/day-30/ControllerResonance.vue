...
import { drawRippleGlassLayer } from './rippleGlassLayer.js'
...
import { createPointerRefraction } from '../day-29/pointerRefraction.js'
import { createPointerFloat } from '../day-28/pointerFloat.js'
import { createResonanceParticles } from '../day-29/resonanceParticles.js'

const root = ref(null)
const glassScene = inject(glassSceneKey, null)
let removeRippleLayer
...
function finishRipple() {
  ...
  nextTick(() => glassScene?.invalidate())
}
...
function render(refreshScene = true) {
  ...
  glassScene?.publish('controller', renderer.domElement)
}
...
onMounted(async () => {
  removeRippleLayer = glassScene?.addOverlay('ripple', (context, rect, width, height) =>
    drawRippleGlassLayer(root.value?.querySelector('.day-30-ripples span'), context, rect, width, height))
...
<template>
  <div ref="root" class="day-30-story">
    <FluidBackground :paused="backgroundPaused || !ready || !!error" />
    <div v-if="rippleActive && !error" :key="rippleRun" class="day-30-ripples" :class="{ 'day-30-ripples-paused': backgroundPaused }" aria-hidden="true">
      <span @animationend="finishRipple"></span>
    </div>
    <div ref="particleViewport" class="day-30-particles" aria-hidden="true"></div>
    <div ref="viewport" class="day-30-viewport"></div>

    <div class="day-30-aura" aria-hidden="true">
      <span></span><span></span><span></span>
    </div>

    <div v-if="!ready || error" class="day-30-loading" role="status" aria-live="polite">
      <span class="day-30-loading-brand">player / one</span>
      <strong>{{ error ? '—' : String(loadProgress).padStart(2, '0') }}<small v-if="!error">%</small></strong>
      <p>{{ error || loading }}</p>
      <div v-if="!error" class="day-30-loading-track">
        <span :style="{ transform: `scaleX(${loadProgress / 100})` }"></span>
      </div>
    </div>

    <aside class="day-30-rail" aria-label="閱讀進度">
      <span class="day-30-rail-label" aria-live="polite">{{ currentLabel }}</span>
      <div class="day-30-rail-track">
    ...
    <section
      ...
      :key="index"
      class="day-30-chapter"
      :class="[`day-30-chapter-${index + 1}`, { 'is-active': activeIndex === index }]"
      :aria-label="chapter.title"
    >
      <div v-if="!chapter.quiet" class="day-30-copy">
        <h2>{{ chapter.title }}</h2>
        <p class="day-30-detail">{{ chapter.detail }}</p>
      </div>
      <div
        v-if="index === 0"
        class="day-30-scroll-cue"
