...
import { createPointerFloat } from './pointerFloat.js'
...
<template>
  <div ref="root" class="day-28-story">
    <div v-if="rippleActive && !error" :key="rippleRun" class="day-28-ripples" :class="{ 'day-28-ripples-paused': backgroundPaused }" aria-hidden="true">
      <span @animationend="finishRipple"></span>
    </div>
    <div ref="viewport" class="day-28-viewport"></div>

    <div class="day-28-aura" aria-hidden="true">
      <span></span><span></span><span></span>
    </div>

    <div v-if="!ready || error" class="day-28-loading" role="status" aria-live="polite">
      <span class="day-28-loading-brand">player / one</span>
      <strong>{{ error ? '—' : String(loadProgress).padStart(2, '0') }}<small v-if="!error">%</small></strong>
      <p>{{ error || loading }}</p>
      <div v-if="!error" class="day-28-loading-track">
        <span :style="{ transform: `scaleX(${loadProgress / 100})` }"></span>
      </div>
    </div>

    <aside class="day-28-rail" aria-label="閱讀進度">
      <span class="day-28-rail-label" aria-live="polite">{{ currentLabel }}</span>
      <div class="day-28-rail-track">
    ...
    <section
      ...
      :key="index"
      class="day-28-chapter"
      :class="[`day-28-chapter-${index + 1}`, { 'is-active': activeIndex === index }]"
      :aria-label="chapter.title"
    >
      <div v-if="!chapter.quiet" class="day-28-copy">
        <h2>{{ chapter.title }}</h2>
        <p class="day-28-detail">{{ chapter.detail }}</p>
      </div>
      <div
        v-if="index === 0"
        class="day-28-scroll-cue"
