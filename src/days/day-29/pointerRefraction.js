...
export function createPointerRefraction(renderer, scene, camera, redraw) {
  ...
  const pass = new ShaderPass({
    uniforms: { tDiffuse: { value: null }, uWaves: { value: waves }, uTime: { value: 0 }, uAspect: { value: 1 }, uOffset: { value: new THREE.Vector2() } },
    ...
    fragmentShader: `uniform sampler2D tDiffuse;
      ...
      void main(){
        ...
        vec2 uv=clamp(sampleUv,vec2(.001),vec2(.999));
  ...
  return {
    // Share the same bounded wave trail with the particle vertex shader.
    waves,
    setOffset(x,y){pass.uniforms.uOffset.value.set(x,y)},
    ...
    resize(width,height){
      ...
      sceneTarget.setSize(Math.max(1,Math.floor(width*ratio)),Math.max(1,Math.floor(height*ratio)))
      sceneDirty=true
    },
    dispose(){disposed=true;cancelAnimationFrame(frame);window.removeEventListener('pointermove',move);window.removeEventListener('blur',reset);document.removeEventListener('visibilitychange',reset);reduced.removeEventListener('change',reset);scenePass.dispose();sceneTarget.dispose();pass.dispose();output.dispose();composer.dispose()}
  }
}
