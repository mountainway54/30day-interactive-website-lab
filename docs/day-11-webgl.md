...
經過鐵人賽的 1/3，我們終於來到 WebGL，可喜可賀！

如同在 Day 7 稍微提過的，WebGL 是 `<canvas>` 提供的另一種繪圖環境，也可以把它理解成瀏覽器中的 GPU 圖形 API。它能將大量的頂點與像素運算交給顯示卡處理，因此特別適合用來呈現 3D 場景、複雜特效，以及大量的圖形資料。

不過在實際開發中，多數情況並不會直接裸寫 WebGL，而是透過 Three.js 等上層函式庫進行開發。因此接下來介紹 WebGL 時，會將重點放在它背後的原理與渲染邏輯，而非實作語法。
...
const vertices = new Float32Array([
  // x      y      r     g     b
  0, 0.78, 0.91, 0.35, 0.68, -0.78, -0.72, 0.22, 0.17, 0.79, 0.78, -0.72, 0.22,
  0.17, 0.79,
]);
```
...
接著呼叫 `drawArrays()`，正式告訴 GPU 開始繪製。GPU 會執行前面設定好的 Shader，經過光柵化等流程，算出每個像素的顏色，最後把繪製結果寫進 Frame Buffer。
